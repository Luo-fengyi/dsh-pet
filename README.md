...
**装扮**：模型自带的头饰、眼镜、桌面道具等配件可以自由组合（六个分类各挑一件，能叠加）。右键一键随机，也能让每次启动都随机换一套。
...
这套模型里带了几十个可切换的配件（就是作者那份按键表里列的东西），本项目把它们按类整理好，
可以**自由组合**——同时戴眼镜、插猫耳、桌上再摆个芭菲都行。

| 分类 | 可选 |
|---|---|
| **头饰** | 摘掉发箍、单边马尾、猫猫贴纸（猫耳）、兔兔贴纸（兔耳）、蝴蝶结贴纸 |
| **眼镜** | 圆眼镜、方眼镜、椭圆眼镜、墨镜 |
| **桌面道具** | 巴菲、蛋包饭、点菜按下、鲸鱼放桌上 |
| **手上/手机** | 魔爪、魔爪换色、喵喵手、双手比耶、手机换色 |
| **背景** | 深色桌布 |
| **小物件** | 画笔、橡皮、撤回、挤、爱心、鲸鱼 |

**怎么用**

- **设置窗 →「装扮」页**：每个分类挑一件（也可以选「不装」），点保存立刻生效
- **右键桌宠 →「随机换装扮」**：立刻随机一套；「恢复默认装扮」全部清空
- **每次启动随机换**：`outfit.startupRandom`（默认开）。关掉就沿用上次那一套

**原理**：装扮本质上是模型里那些 exp3 表情文件，但它们**不能**当普通表情用——
Cubism 一次只允许一个表情生效，叠不起来。所以这里改成把选中的几套参数合并起来、
**每帧覆盖写一遍**，这样既不会互相冲突，也不会被待机动作冲掉。

分类和候选项都写在 `config.json` 的 `outfit.categories` 里，想加就自己往里添名字
（名字要和 `assets/model/manifest.json` 里的 `label` 对得上，也就是 `DS` 目录里那些文件名）。

候选项支持两种写法：直接写名字，或者写成对象给个更好懂的显示名——模型里有些开关名字是反的，
比如「头箍」这个参数实际控制的是**摘掉**发箍（模型默认戴着），所以写成了：

```json
{ "label": "头箍", "name": "摘掉发箍（默认是戴着的）" }
```

`label` 是文件名（必须对得上），`name` 只是设置窗里显示的文字。
